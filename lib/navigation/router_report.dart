...
import 'package:fatima/state_manager/controller/flutter_engine.dart';
import 'package:fatima/state_manager/controller/lifecycle.dart';
import 'package:fatima/core/typedefs.dart';

class RouterReportManager<T> {
  /// Holds a reference to `Fatima.reference` when the Instance was
  /// created to manage the memory.
  final Map<T?, List<String>> _routesKey = {};

  /// Stores the onClose() references of instances created with `Fatima.create()`
  ...
  void printInstanceStack() {
    Fatima.log(_routesKey.toString());
  }
  ...
  void appendRouteByCreate(LifeCycleMixin i) {
    _routesByCreate[_current] ??= HashSet<Function>();
    // _routesByCreate[Fatima.reference]!.add(i.onDelete as Function);
  ...
  void reportRouteWillDispose(T disposed) {
    ...
    /// Removes `Fatima.create()` instances registered in `routeName`.
    ...
    for (final element in keysToRemove) {
      Fatima.markAsDirty(key: element);
  ...
  /// using `Fatima.smartManagement` as [SmartManagement.full] or
  ...
  void _removeDependencyByRoute(T routeName) {
    ...
    for (final element in keysToRemove) {
      final value = Fatima.delete(key: element);
      if (value) {
