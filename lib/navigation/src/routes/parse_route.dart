...
class RouteDecoder {
  ...
  final List<FatimaPage> currentTreeBranch;
  final PageSettings? pageSettings;

  factory RouteDecoder.fromRoute(String location) {
    var uri = Uri.parse(location);
    final args = PageSettings(uri);
    final decoder = (Fatima.rootController.routerDelegate as FatimaDelegate)
  ...
  List<FatimaPage>? get currentChildrens => route?.children;
...
class ParseRouteTree {
  ...
  void addRoutes<T>(List<FatimaPage<T>> getPages) {
    for (final route in getPages) {
      addRoute(route);
    }
  }

  void removeRoutes<T>(List<FatimaPage<T>> getPages) {
    for (final route in getPages) {
      removeRoute(route);
    }
  }

  void removeRoute<T>(FatimaPage<T> route) {
    routes.remove(route);
  ...
  void addRoute<T>(FatimaPage<T> route) {
    routes.add(route);
  ...
  List<FatimaPage> _flattenPage(FatimaPage route) {
    final result = <FatimaPage>[];
  ...
  FatimaPage _addChild(FatimaPage origin, String parentPath,
      List<FatimaMiddleware> middlewares) {
    return origin.copy(
  ...
  FatimaPage? _findRoute(String name) {
