...
class RouterOutlet<TDelegate extends RouterDelegate<T>, T extends Object>
  ...
  })  : routerDelegate = delegate ?? Fatima.delegate<TDelegate, T>()!,
        super();

  RouterOutlet({
    TDelegate? delegate,
    required Iterable<FatimaPage> Function(T currentNavStack) pickPages,
    required Widget Function(
      BuildContext context,
      TDelegate,
      Iterable<FatimaPage>? page,
    )
        pageBuilder,
  }) : this.builder(
          builder: (context) {
...
class GetRouterOutlet extends RouterOutlet<FatimaDelegate, RouteDecoder> {
  GetRouterOutlet({
    String? anchorRoute,
    required String initialRoute,
    Iterable<FatimaPage> Function(Iterable<FatimaPage> afterAnchor)?
        filterPages,
    // GlobalKey<NavigatorState>? key,
    FatimaDelegate? delegate,
  }) : this.pickPages(
          pickPages: (config) {
            Iterable<FatimaPage<dynamic>> ret;
            if (anchorRoute == null) {
          ...
          key: Fatima.nestedKey(anchorRoute)?.navigatorKey,
          delegate: delegate,
        );
  GetRouterOutlet.pickPages({
    Widget Function(FatimaDelegate delegate)? emptyWidget,
    FatimaPage Function(FatimaDelegate delegate)? emptyPage,
    required Iterable<FatimaPage> Function(RouteDecoder currentNavStack)
        pickPages,
    ...
    FatimaDelegate? delegate,
          ...
          pickPages: pickPages,
          delegate: delegate ?? Fatima.rootController.rootDelegate,
        );
...
extension PagesListExt on List<FatimaPage> {
  Iterable<FatimaPage> pickAtRoute(String route) {
    return skipWhile((value) {
      return value.name != route;
    });
  }

  Iterable<FatimaPage> pickAfterRoute(String route) {
