...
class FatimaDelegate extends RouterDelegate<RouteDecoder>
  ...
  final FatimaPage notFoundRoute;

  final List<NavigatorObserver>? navigatorObservers;
  final TransitionDelegate<dynamic>? transitionDelegate;

  final Iterable<FatimaPage> Function(RouteDecoder currentNavStack)?
  ...
  List<FatimaPage> get registeredRoutes => _routeTree.routes;

  void addPages(List<FatimaPage> fatimaPages) {
    _routeTree.addRoutes(fatimaPages);
  }
  ...
  void addPage(FatimaPage getPage) {
    _routeTree.addRoute(getPage);
  }

  void removePage(FatimaPage getPage) {
  ...
  Iterable<FatimaPage> getVisualPages(RouteDecoder? currentHistory) {
  ...
  Future<T?> to<T>(
    ...
    final getPage = FatimaPage<T>(
      name: routeName,
      opaque: opaque ?? true,
      page: page,
      gestureWidth: gestureWidth,
      showCupertinoParallax: showCupertinoParallax,
      popGesture: popGesture ?? Fatima.defaultPopGesture,
      transition: transition ?? Fatima.defaultTransition,
      curve: curve ?? Fatima.defaultTransitionCurve,
      fullscreenDialog: fullscreenDialog,
      bindings: bindings,
      transitionDuration: duration ?? Fatima.defaultTransitionDuration,
      preventDuplicateHandlingMode: preventDuplicateHandlingMode,
    );
  ...
  Future<T?> _replace<T>(PageSettings arguments, FatimaPage<T> page) async {
